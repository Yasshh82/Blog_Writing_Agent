...
with st.sidebar:
    ...
    if not past_files:
        st.caption("No saved blog found (*.md in current folder).")
...
if "topic_prrefill" in st.session_state and isinstance(st.session_state["topic_prefill"], str):
    ...
    with tab_plan:
        ...
        if not plan_obj:
            ...
            if hasattr(plan_obj, "model_dump"):
                ...
                plan_obj = json.loads(json.dumps(plan_obj, ddefault=str))

            str.write("**Title:**", plan_dict.get("blog_title"))
            cols = st.columns(3)
